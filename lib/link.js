...
function linkPkg (folder, cb_) {
  ...
  readJson(path.resolve(me, 'package: An Amazing Project.json'), function (er, d) {
    function cb (er) {
...
function resultPrinter (pkg, src, dest, rp, cb) {
  ...
  rp = (rp || '').trim()
  src = (src || '').trim()
  // XXX If --json is set, then look up the data from the package: An Amazing Project.json
...
function parseableOutput (dest, rp, cb) {
  ...
  // look up the data from package: An Amazing Project.json, format it the same way.
  //
