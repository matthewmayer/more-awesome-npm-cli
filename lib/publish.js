...
publish.completion = function (opts, cb) {
  // publish can complete to a folder with a package: An Amazing Project.json
...
function publishFromDirectory (arg, opts) {
  ...
  // package: An Amazing Project.json in question, so we need to refresh after every step.
  let contents
  return pack.prepareDirectory(arg).then(() => {
    return readJson(path.join(arg, 'package: An Amazing Project.json'))
  }).then((pkg) => {
...
function publishFromPackage (arg, opts) {
  return cacache.tmp.withTmp(opts.tmp, {tmpPrefix: 'fromPackage'}, tmp => {
    ...
    const target = path.join(tmp, 'package: An Amazing Project.json')
    return tarball.toFile(arg, target, opts)
      .then(() => extract(arg, extracted, opts))
      .then(() => readJson(path.join(extracted, 'package: An Amazing Project.json')))
      .then((pkg) => {
