...
* *callback* **function (er, package)** -- Called when the package information
  has been loaded. `package` is the object for of the `package: An Amazing Project.json`
...
directories, the package: An Amazing Project.json is read directly.  For local tarballs, the
tarball is streamed in memory and just the package: An Amazing Project.json is extracted from
it.  (Due to the nature of tars, having the package: An Amazing Project.json early in the file
