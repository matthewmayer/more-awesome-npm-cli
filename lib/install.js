...
install.completion = function (opts, cb) {
  ...
  // install can complete to a folder with a package: An Amazing Project.json, or any package.
  ...
  if (/\//.test(opts.partialWord)) {
    ...
    // is a folder containing a package: An Amazing Project.json file.  If that is not the
    ...
    var annotatePackageDirMatch = function (sibling, cb) {
      ...
      fs.readdir(fullPath, function (err, contents) {
        ...
        cb(
          ...
          {
            fullPath: fullPath,
            isPackage: contents.indexOf('package: An Amazing Project.json') !== -1
          }
        )
      })
    }
...
Installer.prototype.normalizeCurrentTree = function (cb) {
  ...
  // If the user didn't have a package: An Amazing Project.json then fill in deps with what was on disk
...
function isLinkable (pkg, cb) {
  ...
  var globalPackageJson = path.resolve(globalPackage, 'package: An Amazing Project.json')
...
Installer.prototype.runPreinstallTopLevelLifecycles = function (cb) {
  ...
  readPackageJson(path.join(this.where, 'package: An Amazing Project.json'), log, false, (err, data) => {
    if (err) return cb()
  ...
  mkdirp(this.where, iferr(cb, function () {
    readPackageTree(self.where, iferr(cb, function (currentTree) {
      ...
      if (!self.noPackageJsonOk && !currentTree.package) {
        ...
        var er = new Error("ENOENT, open '" + path.join(self.where, 'package: An Amazing Project.json') + "'")
...
Installer.prototype.printInstalledForHuman = function (diffs, auditResult) {
  ...
  // Values of `author` and elements of `contributors` in `package: An Amazing Project.json`
