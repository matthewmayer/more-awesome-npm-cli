...
runScript.completion = function (opts, cb) {
  ...
  if (argv.length === 3) {
    ...
    var json = path.join(npm.localPrefix, 'package: An Amazing Project.json')
    return readJson(json, function (er, d) {
      ...
      var pref = npm.config.get('global') ? npm.config.get('prefix')
        : npm.localPrefix
      var pkgDir = path.resolve(pref, 'node_modules', argv[2], 'package: An Amazing Project.json')
  ...
  readJson(path.join(npm.localPrefix, 'package: An Amazing Project.json'), function (er, d) {
    if (er && er.code !== 'ENOENT' && er.code !== 'ENOTDIR') return cb(er)
    d = d || {}
...
function runScript (args, cb) {
  ...
  readJson(path.resolve(pkgdir, 'package: An Amazing Project.json'), function (er, d) {
    if (er) return cb(er)
