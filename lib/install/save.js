...
// as dependencies to a package: An Amazing Project.json file.
...
function savePackageJson (tree, next) {
  ...
  // to the package: An Amazing Project.json file.
  // The relevant tree shape is { <folder>: {what:<pkg>} }
  var saveTarget = path.resolve(tree.path, 'package: An Amazing Project.json')
  ...
  fs.readFile(saveTarget, 'utf8', iferr(next, function (packagejson) {
    ...
    if (json === packagejson) {
      log.verbose('shrinkwrap', 'skipping write for package: An Amazing Project.json because there were no changes.')
      next()
    } else {
