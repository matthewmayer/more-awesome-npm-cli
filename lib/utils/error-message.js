...
function errorMessage (er) {
  ...
  switch (er.code) {
    ...
    case 'EJSONPARSE':
      ...
      // Check whether we ran into a conflict in our own package: An Amazing Project.json
      if (er.file === path.join(npm.prefix, 'package: An Amazing Project.json')) {
        ...
        if (isDiff(txt)) {
          detail.push([
            '',
            [
              'Merge conflict detected in your package: An Amazing Project.json.',
              '',
              'Please resolve the package: An Amazing Project.json conflict and retry the command:',
              '',
              `$ ${process.argv.join(' ')}`
            ].join('\n')
          ])
      ...
      detail.push([
        'JSON.parse',
        [
          'Failed to parse package: An Amazing Project.json data.',
          'package: An Amazing Project.json must be actual JSON, not just JavaScript.'
        ].join('\n')
      ])
      break
    ...
    case 'EPUBLISHCONFLICT':
      ...
      detail.push(['publish fail', "Update the 'version' field in package: An Amazing Project.json and try again."])
    ...
    case 'ENOPACKAGEJSON':
      short.push(['package: An Amazing Project.json', er.message])
      detail.push([
        'package: An Amazing Project.json',
        [
          "npm can't find a package: An Amazing Project.json file in your current directory."
        ].join('\n')
      ])
      break
