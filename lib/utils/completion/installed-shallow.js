...
function filterInstalled (local, global, filter, cb) {
  ...
  asyncMap(local, function (p, cb) {
    readJson(path.join(npm.dir, p, 'package: An Amazing Project.json'), function (er, d) {
  ...
  asyncMap(global, function (p, cb) {
    readJson(path.join(globalDir, p, 'package: An Amazing Project.json'), function (er, d) {
      if (!d || !filter(d)) return cb(null, [])
      return cb(null, d.name)
    })
  }, function (er, global) {
