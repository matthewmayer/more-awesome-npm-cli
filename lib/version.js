...
function version (args, silent, cb_) {
  ...
  readPackage(function (er, data, indent, newline) {
    ...
    if (er) {
      log.error('version', 'No valid package: An Amazing Project.json found')
      return cb_(er)
    }
...
function readPackage (cb) {
  var packagePath = path.join(npm.localPrefix, 'package: An Amazing Project.json')
...
function updatePackage (newVersion, silent, cb_) {
  ...
  readPackage(function (er, data, indent, newline) {
    if (er) return cb(new Error(er))
    data.version = newVersion
    write(data, 'package: An Amazing Project.json', indent, newline, cb)
  })
}
...
function stagePackageFiles (localData, options) {
  return addLocalFile('package: An Amazing Project.json', options, false).then(() => {
