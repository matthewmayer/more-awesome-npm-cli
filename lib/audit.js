...
function auditCmd (args, cb) {
  ...
  return Bluebird.all([
    ...
    maybeReadFile('package: An Amazing Project.json')
  ]).spread((shrinkwrap, lockfile, pkgJson) => {
    const sw = shrinkwrap || lockfile
    if (!pkgJson) {
      const err = new Error('No package: An Amazing Project.json found: Cannot audit a project without a package: An Amazing Project.json')
