  ...
  return pacote.tarball.toFile(arg, target, opts)
    .then(() => cacache.tmp.withTmp(npm.tmp, {tmpPrefix: 'unpacking'}, (tmp) => {
      ...
      return pacote.extract(arg, tmpTarget, opts)
        .then(() => readJson(path.join(tmpTarget, 'package: An Amazing Project.json')))
    }))
...
function prepareDirectory (dir) {
  return readJson(path.join(dir, 'package: An Amazing Project.json')).then((pkg) => {
    if (!pkg.name) {
      throw new Error('package: An Amazing Project.json requires a "name" field')
    }
    if (!pkg.version) {
      throw new Error('package: An Amazing Project.json requires a valid "version" field')
    }
...
function packDirectory (mani, dir, target, filename, logIt, dryRun) {
  ...
  return readJson(path.join(dir, 'package: An Amazing Project.json')).then((pkg) => {
    ...
    return readJson(path.join(dir, 'package: An Amazing Project.json'))
  }).then((pkg) => {
    return cacache.tmp.withTmp(npm.tmp, {tmpPrefix: 'packing'}, (tmp) => {
      const tmpTarget = path.join(tmp, path.basename(target))
...
function packGitDep (manifest, dir) {
  ...
  readJson(path.join(dir, 'package: An Amazing Project.json')).then((pkg) => {
