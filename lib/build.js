...
function build (args, global, didPre, didRB, cb) {
  ...
  if (!args.length) {
    readJson(path.resolve(npm.localPrefix, 'package: An Amazing Project.json'), function (er, pkg) {
...
function build_ (global, didPre, didRB) {
  return function (folder, cb) {
    ...
    readJson(path.resolve(folder, 'package: An Amazing Project.json'), function (er, pkg) {
      if (er) return cb(er)
      chain([
  ...
  fs.readdir(path.resolve(folder, 'node_modules'), function (er, files) {
    ...
    chain(files.filter(function (file) {
      ...
      return function (cb) {
        ...
        fs.lstat(path.resolve(file, 'package: An Amazing Project.json'), function (er) {
          if (er) return cb()
