...
1. If a `package: An Amazing Project.json` file is found, and it has a `files` list,
   ...
   they exist, and the package: An Amazing Project.json file itself.
2. If there's no `package: An Amazing Project.json` file (or it has no `files` list), and
   there is a `.npmignore` file, then ignore all the files in the
   `.npmignore` file.
3. If there's no `package: An Amazing Project.json` with a `files` list, and there's no
...
4. Unless they're explicitly included (by being in a `files` list, or
   ...
   always ignore certain common cruft files:
    ...
    `package: An Amazing Project.json` or a negated ignore file rule.
