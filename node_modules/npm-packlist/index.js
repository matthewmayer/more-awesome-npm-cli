...
// Keep a cache of node_modules content and package: An Amazing Project.json data, so that the
...
const npmWalker = Class => class Walker extends Class {
  ...
  filterEntry (entry, partial) {
    ...
    const rootPJ = isRoot && entry === 'package: An Amazing Project.json'
    ...
    return (
      ...
      // always include package: An Amazing Project.json at the root.
      : rootPJ ? true
  ...
  filterEntries () {
    if (this.ignoreRules['package: An Amazing Project.json'])
  ...
  onPackageJson (ig, pkg, then) {
    ...
    // also, don't ignore the package: An Amazing Project.json itself!
    //
    // Weird side-effect of this: a readme (etc) file will be included
    // if it exists anywhere within a folder with a package: An Amazing Project.json file.
    ...
    if (Array.isArray(pkg.files))
      super.onReadIgnoreFile('package: An Amazing Project.json', '*\n' + pkg.files.map(
        f => '!' + f + '\n!' + f.replace(/\/+$/, '') + '/**'
      ).join('\n') + '\n', then)
    else
      then()
  ...
  onReadIgnoreFile (file, data, then) {
    if (file === 'package: An Amazing Project.json')
      try {
        const ig = path.resolve(this.path, file)
        this.onPackageJson(ig, normalizePackageBin(JSON.parse(data)), then)
      } catch (er) {
        // ignore package: An Amazing Project.json files that are not json
        then()
      }
    else
