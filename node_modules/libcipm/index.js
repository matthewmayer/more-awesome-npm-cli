...
class Installer {
  ...
  prepare () {
    ...
    return (
      ...
      .then(prefix => {
        ...
        return BB.join(
          readJson(prefix, 'package: An Amazing Project.json'),
  ...
  checkLock () {
    ...
    return lockVerify(prefix).then(result => {
      if (result.status) {
        ...
        throw new Error(
          'cipm can only install packages when your package: An Amazing Project.json and package-lock.json or ' +
  ...
  extractTree (tree) {
    ...
    return tree.forEachAsync((dep, next) => {
      ...
      if (dep.isRoot) {
        ...
        return (
          dep.bundled
            ? statAsync(path.join(depPath, 'package: An Amazing Project.json')).catch(err => {
  ...
  updateJson (tree) {
    ...
    return tree.forEachAsync((dep, next) => {
      ...
      const depPath = dep.path(this.prefix)
      return next()
        .then(() => readJson(depPath, 'package: An Amazing Project.json'))
    ...
    return tree.forEachAsync((dep, next) => {
      ...
      // Don't link root bins
        .then(() => {
          ...
          return readPkgJson(path.join(depPath, 'package: An Amazing Project.json'))
  ...
  updateFromField (dep, pkg) {
    const depPath = dep.path(this.prefix)
    const depPkgPath = path.join(depPath, 'package: An Amazing Project.json')
    const parent = dep.requiredBy.values().next().value
    return readJson(parent.path(this.prefix), 'package: An Amazing Project.json')
      .then(ppkg =>
