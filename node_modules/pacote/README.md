...
on the `package: An Amazing Project.json` for that package, but with pre-processed and limited
...
pacote.manifest('pacote@1.0.0').then(pkgJson => {
  // fetched `package: An Amazing Project.json` data from the registry
})
```
