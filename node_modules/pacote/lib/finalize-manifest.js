...
function tarballedProps (pkg, spec, opts) {
  ...
  if (!needsShrinkwrap && !needsBin && !needsHash && !needsManifest) {
    ...
    return BB.join(
      ...
      needsManifest && jsonFromStream('package: An Amazing Project.json', extracted),
      ...
      (sr, mani, paths, hash) => {
        if (needsManifest && !mani) {
          const err = new Error(`Non-registry package missing package: An Amazing Project.json: ${spec}.`)
